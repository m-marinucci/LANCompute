...
class MacOptimizer:
    ...
    def _detect_unified_memory(self) -> Dict[str, Any]:
        ...
        try:
            ...
            memory_info["memory_bandwidth_gbps"] = self._estimate_memory_bandwidth(
                chip_model
            )
            ...
            if self.is_apple_silicon:
                try:
                    ...
                    cpu_info["efficiency_cores"] = (
                        cpu_info["physical_cores"] - cpu_info["performance_cores"]
                    )
    ...
    def get_system_summary(self) -> str:
        ...
        if self.unified_memory_info["has_unified_memory"]:
            lines.append(
                f"Memory Bandwidth: {self.unified_memory_info['memory_bandwidth_gbps']:.1f} GB/s"
            )
        else:
            lines.append(
                f"GPU Memory: {self.unified_memory_info['gpu_memory_gb']:.1f} GB"
            )
