    ...
    def get_node_if_available(self, node_id: str, max_tasks: int = 2) -> Optional[Node]:
        ...
        with self.lock:
            if node_id in self.nodes:
                ...
                if (
                    node.status == NodeStatus.ONLINE
                    and len(node.current_tasks) < max_tasks
                ):
                    return node
            return None
...
class MasterHTTPHandler(BaseHTTPRequestHandler):
    ...
    def _handle_register_node(self, data: Dict[str, Any]):
        ...
        try:
            ...
            self._send_json_response({"node_id": node.id, "status": "registered"})
        except KeyError as e:
            self.send_error(400, f"Missing required field: {e}")
    ...
    def _handle_heartbeat(self, data: Dict[str, Any]):
        ...
        if success:
            ...
            if node:
                ...
                if task:
                    self.server.master.node_manager.assign_task_to_node(
                        node_id, task.id
                    )
                    self._send_json_response(
                        {"status": "ok", "task": serialize_dataclass(task)}
                    )
                    return
    ...
    def _handle_update_task(self, data: Dict[str, Any]):
        ...
        success = self.server.master.task_queue.update_task_status(
            task_id, task_status, result=data.get("result"), error=data.get("error")
        )
