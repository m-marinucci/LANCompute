    ...
    def get_capabilities() -> Dict[str, Any]:
        ...
        if platform.system() == "Darwin" and MacOptimizer:
            try:
                ...
                capabilities["unified_memory"] = optimizer.unified_memory_info[
                    "has_unified_memory"
                ]
                capabilities["unified_memory_gb"] = optimizer.unified_memory_info[
                    "total_memory_gb"
                ]
                capabilities["gpu_cores"] = optimizer.gpu_info["gpu_cores"]
                capabilities["metal_support"] = optimizer.gpu_info["metal_support"]
                capabilities["neural_engine"] = (
                    optimizer.gpu_info["neural_engine_cores"] > 0
                )

                # Core types for Apple Silicon
                if optimizer.is_apple_silicon:
                    capabilities["performance_cores"] = optimizer.cpu_info[
                        "performance_cores"
                    ]
                    capabilities["efficiency_cores"] = optimizer.cpu_info[
                        "efficiency_cores"
                    ]
        ...
        capabilities["tensorflow_available"] = PlatformDetector._check_package(
            "tensorflow"
        )
    ...
    def _detect_gpu() -> bool:
        ...
        if platform.system() == "Linux":
            try:
                ...
                if "VGA" in result.stdout and (
                    "AMD" in result.stdout or "ATI" in result.stdout
                ):
                    return True
            except (subprocess.SubprocessError, OSError, FileNotFoundError):
                pass
...
class WorkerService:
    ...
    def start(self):
        ...
        logger.info(
            f"Platform: {self.capabilities.get('platform')} "
    ...
    def _register(self) -> bool:
        ...
        try:
            ...
            response = self.session.post(
                f"{self.config.master_url}/node/register", json=data, timeout=5
            )
    ...
    def _update_task_status(
        ...
        try:
            ...
            response = self.session.post(
                f"{self.config.master_url}/task/update", json=data, timeout=5
            )
...
def main():
    ...
    parser.add_argument(
        "--max-tasks", type=int, default=2, help="Maximum concurrent tasks"
    )
    parser.add_argument(
        "--heartbeat-interval",
        type=float,
    ...
    parser.add_argument(
        "--max-workers", type=int, default=None, help="Maximum worker threads/processes"
    )
    parser.add_argument(
        "--log-level",
        default="INFO",
